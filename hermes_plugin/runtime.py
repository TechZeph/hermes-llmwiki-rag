"""Plugin runtime: maps native Hermes settings onto the llmwiki core.

The runtime owns no Hermes imports. It validates configuration into an
immutable :class:`llmwiki.config.Settings`, lazily loads the local
models, opens one SQLite connection per call, and tracks the single
background reindex job. Every public method returns plain dictionaries
that the tool layer serialises; none of them raise for expected
...
from llmwiki.citations import build_context
from llmwiki.confidence import Decision, InjectionGate, decide, load_gate
from llmwiki.config import Settings
from llmwiki.embeddings import Embedder
from llmwiki.indexer import Indexer
from llmwiki.models import RetrievalResult
...
def build_settings(config: PluginConfig) -> Settings:
    if not config.vault:
        raise ConfigError("plugins.entries.llmwiki.settings.vault is not set")
    ...
    def status(self) -> dict[str, Any]:
        if not self.configured:
            return {
                ...
                "remediation": "set plugins.entries.llmwiki.settings.vault to the vault path",
            }
        settings = self.settings
